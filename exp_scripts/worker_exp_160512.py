...
class WorkerIterativeLinearSystemSolverCG_Exp_160507_A(Worker):
    ...
    def conduct_experiments(self, block_size, tol, maxiter, step_val):
        ...
        self._bcbcg_exp()
        self._try_matplotlib_simple_draw(self._residual_hist)
    ...
    def _try_matplotlib_simple_draw(self, y_data):
        Y_data = np.asarray([ math.log(y,10) for y in y_data]);
        X_data=np.asarray( [x for x in range(0,len(y_data))] )
