...
class Presenter():
    ...
    def instant_plot_unified_x_axis (self, X_data, Y_data_list, figure_title, x_label, y_label,legend_list, color_list):
        """ plot with matplotlib"""
        for idx in range(0,len(Y_data_list)):
            #plt.plot(X_data, Y_data_list[idx], label=legend_list[idx] ,c=color_list[idx])
            plt.plot(X_data, Y_data_list[idx], label=legend_list[idx] )
        ...
        plt.ylabel(y_label, fontsize=30)
        #plt.ylim(-4,4)
        plt.legend(fancybox=True, prop={"size":16},loc="lower right")
        plt.grid(True, lw=2, ls="--", c=".75")
        plt.show()
    ...
    def instant_plot_y_log10 (self, y_data_list, figure_title, x_label, y_label,legend_list, color_list):
        ...
        for idx in range(0,len(y_data_list)):
            ...
            X_data = np.asarray( [x for x in range(0,len(y_data_list[idx]))] )
            #plt.plot(X_data, Y_data, label=legend_list[idx] ,c=color_list[idx])
            plt.plot(X_data, Y_data, label=legend_list[idx] ,c=color_list[idx], linewidth=3)
        ...
        plt.xticks(np.arange(min_x_axis, max_x_axis+1, 10.0))
        plt.yticks(np.arange(min_y_axis, max_y_axis+1, 1.))
        plt.tick_params(axis="both", which="major", labelsize=35)
        #plt.tick_params(axis="both", which="minor", labelsize=10)
        plt.ylim(min_y_axis-0.5, max_y_axis)
        plt.title(figure_title, fontsize=80)
        plt.xlabel(x_label, fontsize=60)
        plt.ylabel(y_label, fontsize=60)
        plt.legend(fancybox=True, prop={"size":60})
        plt.grid(True, lw=2.5, ls="--", c=".75")
        plt.show()
